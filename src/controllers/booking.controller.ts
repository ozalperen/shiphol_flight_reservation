...
export const createBookingHandler = async (
  ...
  try {
    ...
    if (updatedFlight.scheduleDateTime < new Date(Date.now())) {
      return next(new AppError(400, "Cannot book for past flights"));
    }
    if (updatedFlight.flightDirection === "A") {
      return next(new AppError(400, "Cannot book for arrivals"));
    }
    if (updatedFlight.avalibleSeats.includes(req.body.seatNumber) === false) {
      return next(new AppError(400, "Chosen seat already booked"));
    }
